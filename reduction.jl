...
get_effector_percent_engaged(sol) = get_percent_engaged(idx_n_effector_free(), sol)
get_target_percent_engaged(sol) = get_percent_engaged(idx_n_target_free(), sol)
function get_percent_engaged(idx, sol)
...
function reduced_liu_model!(du, u, p, t)
    ...
    # Apply CD3 downregulation and CD19 internalisation.
    TCR_ratio = TCR_hill(p.equilibrium_TCR_binary, t)
    TCR_free   = p.equilibrium_TCR_free   * TCR_ratio
    TCR_binary = p.equilibrium_TCR_binary * TCR_ratio
...
function calculate_binding_probability!(TCR_free, TCR_binary, TAA_free, TAA_binary, p)
    ...
    # Liu originally samples a random number between 0.1 and 5.0.
    # This uses the mean of that distribution. 
    ...
    # But, for consistency we will be solving the ODEs. 
    prob = remake(p.cache.prob; u0=p.cache.xstart, tspan = (0.0, bondtime))
    solver = BS3() # TRBDF2()
    sol = solve(prob, solver; abstol=1e-3, reltol=1e-2,
